...
function rootGenerate() {
  Promise.all([mkDir(`controllers`), mkDir(`models`), mkDir(`routes`), mkDir(`spec`)])
  .then(() => {
    return Promise.all([mkFile(`app.js`, app), mkFile(`listen.js`, listen), mkFile(`routes/api.js`, api), mkFile(`spec/app.spec.js`, appSpec)])
  })
  .then(() => {
    return install(`npm init -y && npm i -s express body-parser && npm i -D chai supertest`)
  })
  .catch(err => {
    console.log(`Error in making the project: ${err}`)
  });
